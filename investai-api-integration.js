...
async function cargarMercado(ticker = "FSM") {
  ...
  try {
    ...
    // OPCIÓN B: Endpoint dinámico del backend (descomenta cuando el backend esté activo)
    const allData = await apiFetch(`/api/mercado`);
    // Si tu endpoint devuelve un ticker individual, usa:
